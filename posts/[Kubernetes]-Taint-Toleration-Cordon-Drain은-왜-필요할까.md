---
title: "[Kubernetes] Taint & Toleration, Cordon & Drain은 왜 필요할까?"
source: "https://velog.io/@yorange50/Kubernetes-Taint-Toleration-Cordon-Drain%EC%9D%80-%EC%99%9C-%ED%95%84%EC%9A%94%ED%95%A0%EA%B9%8C"
published: "Fri, 29 May 2026 05:43:35 GMT"
backup_date: "2026-05-29T14:51:00.446481"
---
...
effect | 의미  
---|---  
NoSchedule | toleration이 없는 Pod는 새로 배치하지 않음  
PreferNoSchedule | 가능하면 배치하지 않음  
NoExecute | toleration이 없는 기존 Pod도 내보냄  
...
그래서 특정 Node로 보내고 싶으면 보통 `nodeSelector`나 `nodeAffinity`를 같이 쓴다.
...
그러면 `worker-1`은 SchedulingDisabled 상태가 된다.
...
이제 Scheduler는 새 Pod를 `worker-1`에 배치하지 않는다.
...
즉 이미 `worker-1`에서 실행 중인 Pod는 그대로 있다.
...
예를 들어 `worker-1`을 재부팅하거나, 패키지를 업데이트하거나, 장애가 의심되어 새 Pod 유입을 막고 싶다고 하자.
...
그러면 쿠버네티스는 `worker-1`에 있던 Pod들을 삭제한다.
...
이때 `--ignore-daemonsets`를 붙여서 말한다.
...
`emptyDir`은 Pod가 삭제되면 같이 사라지는 임시 저장소이기 때문이다.
...
새 Pod 스케줄링 차단 | 함 | 함  
기존 Pod 제거 | 안 함 | 함  
주 목적 | 새 Pod 유입 차단 | Node 비우기  
사용 상황 | 점검 전 준비, 장애 의심 Node 격리 | Node 재부팅, 업그레이드, 교체 전  
되돌리기 | uncordon | 작업 후 uncordon 필요  
...
구분 | taint/toleration | cordon/drain  
---|---|---  
목적 | 특정 Pod만 특정 Node에 들어오게 제어 | Node 점검/장애 시 Pod 이동  
적용 대상 | Node와 Pod의 스케줄링 조건 | 특정 Node의 운영 상태  
지속성 | 정책처럼 계속 유지 가능 | 보통 점검 중 임시 사용  
기존 Pod 영향 | effect에 따라 다름 | drain은 기존 Pod 제거  
대표 상황 | GPU Node, 전용 Node, control-plane 보호 | Node 재부팅, 업그레이드, 장애 격리  
