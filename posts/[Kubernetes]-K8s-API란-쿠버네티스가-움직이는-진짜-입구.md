...
source: "https://velog.io/@yorange50/Kubernetes-K8s-API%EB%9E%80-%EC%BF%A0%EB%B2%84%EB%84%A4%ED%8B%B0%EC%8A%A4%EA%B0%80-%EC%9B%80%EC%A7%81%EC%9D%B4%EB%8A%94-%EC%A7%84%EC%A7%9C-%EC%9E%85%EA%B5%AC"
published: "Thu, 28 May 2026 23:58:39 GMT"
backup_date: "2026-05-29T14:51:00.464834"
---
...
Pod를 만들 때도, Deployment를 수정할 때도, Service를 삭제할 때도, Node 상태를 조회할 때도 마찬가지다.
...
K8s API는 “어떤 방식으로 요청을 보내야 하는지”에 대한 규칙이다. kube-apiserver는 그 요청을 실제로 받아서 처리하는 서버다.
...
사용자는 `spec`에 원하는 상태를 적는다. 쿠버네티스는 `status`를 보면서 실제 상태를 원하는 상태에 맞춘다.
...
`kubectl`도 API Server를 보고, Controller도 API Server를 보고, Scheduler도 API Server를 보고, kubelet도 API Server와 통신한다.
...
> Kubernetes API를 통해 원하는 상태를 선언하고, 쿠버네티스가 그 상태를 맞춰가는 과정을 이해하는 것
